...
"""
Test à réaliser lors de l'execution ce mercredi :

 - verifier contenu de la liste "self.liste_input_CSV" ligne 210 -> que les ligne ecrites dans le csv soient cohérentes et correspondent bien au calculs voulue (ecriture toutes les 3 minutes)
 - dans la meme veine, verifier le contenu des deux df de la classe LastDataDf à chaque itération : lignes 105 à 107

Vérifier le bon fonctionnement général du code.


Tests et vérification exclusifs au graphe d'état :
	- tester les différents états
	- tester la réaction des composant en fonction des états simulés
	
Revoir le code en case de prolème sur ces tests



A completer en terme de code : 
		revoir la luminosité pour le TVoC moyen et l'Humidity moyenne (code non fait)
		
		revoir les seuil pour les particules et la température encas de danger mortel (incendie, fuite de gaz, etc...)
...
class Gateway:
	def __init__(self):
		...
		self.delta_time_writing  = 180 # temps en secondes
	...
	def graph(self):
		...
		if self.current_datetime.isoweekday() == 7 or self.current_datetime.time() > dt.datetime(2024,1,1,18,30,00).time() :
			self.etat = 2
		# cas du samedi
		elif self.current_datetime.isoweekday() == 6 or self.current_datetime.time() > dt.datetime(2024,1,1,12,30,00).time() :
			self.etat = 2
